...
const MovieInfoSection = ({ movie, categories }) => {
  ...
  const handleAddFavorite = async () => {
    ...
    try {
      ...
      toast.error(err?.message || "Error adding favorite");
    }
  };
  ...
  const handleDeleteFavorite = async () => {
    ...
    try {
      ...
      toast.error(err?.message || "Error deleting favorite");
    }
  };
      ...
      <div className="flex flex-col grow-0 shrink flex-nowrap gap-8">
        <div className="flex flex-col grow-0 shrink flex-nowrap gap-4">
          <div className="items-start grow-0 shrink flex-nowrap">
            <h1 className="text-white text-left text-[32px] font-bold">
              {movie.title}
            </h1>
          </div>
        ...
        <div className="flex flex-row grow-0 shrink flex-nowrap h-12 gap-1">
          <button className="relative inline-flex grow-0 shrink justify-center px-4 text-text-on-focus bg-background-focus rounded-search">
            <Link
            ...
            >
              ...
              <span className="inline-block overflow-hidden text-base font-semibold">
                Watch Free
              </span>
            </Link>
          </button>
          <div className="flex flex-row grow-0 shrink flex-nowrap gap-2 justify-center items-center max-h-12">
            <div
              className="p-3 movie-btn rounded-search max-h-12"
