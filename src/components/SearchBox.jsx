  ...
  return (
    <div className="relative flex flex-row w-full max-w-md">
      ...
      <div className="search-btn-wrapper">
        {query.length > 0 ? (
          <ClearIcon
            className="rounded-search hover:bg-white hover:text-black-100"
            onClick={() => setQuery("")}
          />
        ) : (
          <SearchIcon />
      ...
      {results.length > 0 && (
        <ul className="absolute top-full left-0 z-50 bg-menu rounded-b-sm text-text-default text-sm font-normal mt-1 shadow-lg w-[400px] max-h-[380px] overflow-y-auto">
          {results.map((movie) => (
              ...
              >
                <div className="flex flex-row grow-0 shrink flex-nowrap justify-normal gap-3 items-center">
                  ...
                  <div className="flex flex-col grow-0 shrink flex-nowrap justify-normal">
                    <span className="inline-block text-ellipsis text-white text-sm font-normal max-w-full overflow-hidden wrap-break-word">
                      {movie.title}
                    </span>
                    <span className="inline-block text-ellipsis text-text-mute text-sm font-normal max-w-full overflow-hidden wrap-break-word">
                      {movie.review}
                    </span>
                  </div>
                </div>
